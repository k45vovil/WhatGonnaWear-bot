...
def get_weather(city):
    ...
    try:
        ...
        if feels_like < -25:
            advice = 'Рекомендую☝🏻: Краще сиди вдома умаляю... 🏠 Ну, чи дуже тепло одягайся!'
        elif -25 <= feels_like < -20:
            advice = 'Рекомендую☝🏻: Не забудь про термобілизну! ❄️ Ппц зима лютая 🌨...'
        elif -20 <= feels_like < -15:
            advice = 'Рекомендую☝🏻: Мсьє, діставай пуховик і тепле взуття ❄️'
        elif -15 <= feels_like < -10:
            advice = 'Рекомендую☝🏻: Теплий шарф 🧣 та светр замінить кружку гарячої кави ☕️.'
        elif -10 <= feels_like < -5:
            advice = 'Рекомендую☝🏻: Бррр зимно... 🧦 Не забудь рукавички та шапку!'
        elif -5 <= feels_like < 0:
            advice = 'Рекомендую☝🏻: Тепла куртка та стільовий шарф сьогодні як ніколи до речі!🧣'
        elif 0 <= feels_like < 5:
            advice = 'Рекомендую☝🏻: Улюблений світшот чи теплий спортивний костюм не дадуть замерзнути дупі🍑'
        elif 5 <= feels_like < 10:
            advice = 'Рекомендую☝🏻: Ну дуууже легенька курточка тобі сьогодні не завадить💁'
        elif 10 <= feels_like < 15:
            advice = 'Рекомендую☝🏻: Вигулюємо нові кроси 👟 та худі'
        elif 15 <= feels_like < 20:
            advice = 'Рекомендую☝🏻: Знімай нафіг верхній одяг! Час спортивок 👕 та кофт'
        elif 20 <= feels_like < 25:
            advice = 'Рекомендую☝🏻: Сьогодні шортики 🩳 та майка - пощебече твої яйка!'
        elif 25 <= feels_like:
            advice = 'Рекомендую☝🏻: Мінімум шмоток - максимум засмаги, кабанчик 🌞🏖'

        res = f'{advice}\n\n📍Погода у - м. {city} в {time}:00\n{description}. Температура повітря: {temp}°, відчувається як: {feels_like}°. Вологість {humidity} %, швидкість вітру {wind_speed} м/с.'
        ...
        res2 = f'📍Погода у - м. {city} в {time2}:00\n{description2}. Температура повітря: {temp2}°, відчувається як: {feels_like2}°. Вологість {humidity2} %, швидкість вітру {wind_speed2} м/с.'
        ...
        res3 = f'📍Погода у - м. {city} в {time3}:00\n{description3}. Температура повітря: {temp3}°, відчувається як: {feels_like3}°. Вологість {humidity3} %, швидкість вітру {wind_speed3} м/с.'
